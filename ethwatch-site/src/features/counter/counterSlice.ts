  ...
  reducers: {
    confirmation: (state, action: PayloadAction<Event>) => {
	  ...
	  const event = action.payload
		...
		console.log(`Event seen: ${event.parsed.name}`)
	  ...
	  Object.keys(event.parsed.args).forEach((key) => {
		  args.set(key, event.parsed.args.get(key).toString())
	  })
      ...
      state.list.push({
		  ...
		  confirmations: event.confirmations.size, // TODO: increment
		  accepted: event.accepted,
		  requiredConfirmations: 0, // TODO
		  args,
	  })
    },
  },
...
export const { confirmation } = counterSlice.actions;
