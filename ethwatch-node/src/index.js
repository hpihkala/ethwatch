...
const AbiDB = require('./abiDB')
const blockExplorers = require('./blockExplorers')
const log = require('./log')

// Minimum time to re-check the block explorer for the ABI even if it wasn't there last time
...
const provider = new ethers.providers.WebSocketProvider(process.env.HANDLER_WS_RPC)

const sequelize = new Sequelize(
	'abidb', '', '', 
	{
		dialect: 'sqlite',
		storage: process.env.DB_FILE || 'abis.db',
		dialectOptions: {
			mode: SQLite.OPEN_READWRITE | SQLite.OPEN_CREATE | SQLite.OPEN_FULLMUTEX,
		},
		logging: false,
})
const abiDB = new AbiDB(sequelize)
...
async function getAbiForAddress(address) {
	// Check DB
	let abiObject = await abiDB.retrieve(address)

	// Never checked or check expired? Then retrieve ABI from block explorer
	if (!abiObject || (!abiObject.abi && Date.now() - abiObject.timestamp.getTime() >= RECHECK_INTERVAL)) {
		//log(`Retrieving ABI from block explorer for address ${address}`)
		const startTime = Date.now()
		try {
			stats.apiCalls += 1
			const abi = await blockExplorers[process.env.HANDLER_CHAIN](address)

			//log(`ABI retrieved from block explorer for address ${address} (took ${Date.now() - startTime} ms): ${abi}`)
			abiObject = await abiDB.store(address, abi)
		} catch (err) {
			//log(`ERROR: ${err}`)
			if (err instanceof blockExplorers.APIRateLimitError) {
				stats.rateLimited += 1
			} else {
				stats.errors += 1
			}
		...
		// log(`ABI found in local database for address ${address}`)
		stats.cacheHits += 1
	}
 ...
 */
async function handleLogEntry(logEvent) {
	let abi = await getAbiForAddress(logEvent.address)
	if (!abi) {
		// log(`ABI not found for contract ${logEvent.address}, skipping`)
		stats.notFound += 1
		return
	}
...
const queue = async.queue(async ({ logEvent }, callback) => {
	await handleLogEntry(logEvent)
	callback()
}, process.env.FETCH_CONCURRENCY ? parseInt(process.env.FETCH_CONCURRENCY) : 5)


const main = async () => {
	log('Listening for blocks')

	provider.on({}, async (logEvent) => {
		stats.events += 1
		queue.push({
			logEvent,
		})
...
resetStats()
setInterval(() => {
	log(`Events: ${stats.events}, Successes: ${stats.successes}, Cache hits: ${stats.cacheHits}, API calls: ${stats.apiCalls}, ABI not found: ${stats.notFound}, Event not found: ${stats.eventNotInAbi}, Rate limited: ${stats.rateLimited}, Other errors: ${stats.errors}, Queue: ${queue.length()}`)
	resetStats()
}, 60 * 1000)